...
function AppShellBody() {
    ...
    void fetch(apiPath("/api/sessions"))
      ...
      .then((d) => {
        ...
        if (new URLSearchParams(window.location.search).get("session") !== s.id) {
          router.replace(appUrl({ project: projectKey, session: s.id }), { scroll: false });
        }
      })
      .catch(() => {
  ...
  const handleCwdChange = useCallback((cwd: string | null, projectRoot?: string | null) => {
    ...
    router.replace(appUrl({ project: newProject }), { scroll: false });
  ...
  const handleSelectSession = useCallback((session: SessionInfo, isRestore = false) => {
    ...
    if (!isRestore) {
      router.replace(appUrl({ project: session.projectRoot ?? session.cwd, session: session.id }), { scroll: false });
    }
  ...
  const handleNewSession = useCallback((_sessionId: string, cwd: string) => {
    ...
    setActiveTopPanel(null);
    if (isMobile) setSidebarOpen(false);
    router.replace(appUrl({ project: cwd }), { scroll: false });
  ...
  const handleSessionForked = useCallback((newSessionId: string) => {
    ...
    router.replace(appUrl({ project: activeProjectRootRef.current, session: newSessionId }), { scroll: false });
  }, [invalidateWorkspaceRestore, router, hydrateSelectedSession]);
  ...
  const handleSessionDeleted = useCallback((sessionId: string) => {
    ...
    if (selectedSession?.id === sessionId) {
      ...
      setSystemPrompt(null);
      setActiveTopPanel(null);
      router.replace(appUrl({ project: selectedSession.projectRoot ?? selectedSession.cwd }), { scroll: false });
    }
