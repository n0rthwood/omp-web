...
test("isValidMachineId mirrors the machine id shape rule", async () => {
  const subject = await loadSubject();
  assert.equal(subject.isValidMachineId("gpu-1"), true);
  assert.equal(subject.isValidMachineId("a"), true);
  assert.equal(subject.isValidMachineId("x".repeat(39)), true);
  assert.equal(subject.isValidMachineId(""), false);
  assert.equal(subject.isValidMachineId("-leading"), false);
  assert.equal(subject.isValidMachineId("trailing-"), false);
  assert.equal(subject.isValidMachineId("Upper"), false);
  assert.equal(subject.isValidMachineId("x".repeat(40)), false);
  assert.equal(subject.isValidMachineId("has space"), false);
});

test("toUserVisibleMachine omits baseUrl and headerNames", async () => {
  const subject = await loadSubject();
  const safe = {
    id: "gpu-1",
    name: "GPU Box",
    baseUrl: "https://gpu1.example",
    authMode: "bearer",
    hasCredential: true,
    headerNames: ["X-Cluster"],
    createdAt: "2026-01-01T00:00:00.000Z",
    updatedAt: "2026-01-01T00:00:00.000Z",
    isLocal: false,
  };
  const visible = subject.toUserVisibleMachine(safe);
  ...
  assert.equal("headerNames" in visible, false);
});

// --- fallbackUrls (issue #37) ----------------------------------------------------

test("create and update accept fallbackUrls, normalize each to an origin, and project them without leaking credentials", async () => {
  const { dir, file } = tempMachinesFile();
  try {
    await withEnv({ OMP_WEB_MACHINES_FILE: file }, async () => {
      const subject = await loadSubject();
      const machine = subject.createMachine({
        ...sampleInput(),
        fallbackUrls: ["https://fallback-a.example/some/path?x=1", "https://fallback-b.example"],
      });
      assert.deepEqual(machine.fallbackUrls, ["https://fallback-a.example", "https://fallback-b.example"]);

      const safe = subject.toSafeMachine(machine);
      assert.deepEqual(safe.fallbackUrls, ["https://fallback-a.example", "https://fallback-b.example"]);
      assert.equal(safe.activeUrl, machine.baseUrl); // never probed yet: defaults to the primary
      assert.deepEqual(safe.endpoints, [
        { url: machine.baseUrl, healthy: null },
        { url: "https://fallback-a.example", healthy: null },
        { url: "https://fallback-b.example", healthy: null },
      ]);
      assert.equal(JSON.stringify(safe).includes("hunter2"), false);

      const updated = subject.updateMachine(machine.id, { fallbackUrls: ["https://only-fallback.example"] });
      assert.deepEqual(updated.fallbackUrls, ["https://only-fallback.example"]);

      const cleared = subject.updateMachine(machine.id, { fallbackUrls: [] });
      assert.deepEqual(cleared.fallbackUrls, []);
...
test("fallbackUrls validation rejects non-array input and non-URL entries", async () => {
  const { dir, file } = tempMachinesFile();
  try {
    await withEnv({ OMP_WEB_MACHINES_FILE: file }, async () => {
      const subject = await loadSubject();
      assert.throws(
        () => subject.createMachine({ ...sampleInput(), fallbackUrls: "not-an-array" }),
        (e) => e instanceof subject.MachineValidationError && e.field === "fallbackUrls",
      );
      assert.throws(
        () => subject.createMachine({ ...sampleInput(), fallbackUrls: ["not a url"] }),
        (e) => e instanceof subject.MachineValidationError && e.field === "fallbackUrls",
      );
      assert.throws(
        () => subject.createMachine({ ...sampleInput(), fallbackUrls: ["ftp://example.com"] }),
        (e) => e instanceof subject.MachineValidationError && e.field === "fallbackUrls",
      );
      const machine = subject.createMachine(sampleInput());
      assert.throws(
        () => subject.updateMachine(machine.id, { fallbackUrls: [42] }),
        (e) => e instanceof subject.MachineValidationError && e.field === "fallbackUrls",
      );
    });
  } finally {
...
test("backward compatibility: a machine persisted before fallbackUrls existed migrates to an empty array and behaves exactly as before", async () => {
  const { dir, file } = tempMachinesFile();
  try {
    // Hand-written, exactly the pre-#37 on-disk shape — no `fallbackUrls` key at all.
    ...
    await withEnv({ OMP_WEB_MACHINES_FILE: file }, async () => {
      const subject = await loadSubject();
      const machine = subject.getMachine("legacy-box");
      assert.deepEqual(machine.fallbackUrls, []);

      const safe = subject.toSafeMachine(machine);
      assert.equal(safe.activeUrl, "http://legacy.example");
      assert.deepEqual(safe.endpoints, [{ url: "http://legacy.example", healthy: null }]);
      assert.deepEqual(safe.fallbackUrls, []);

      // A subsequent write (any mutation) re-persists the migrated shape at 0600.
      subject.updateMachine("legacy-box", { name: "Legacy Box Renamed" });
      assert.equal(statSync(file).mode & 0o777, 0o600);
      const onDisk = JSON.parse(readFileSync(file, "utf8"));
      assert.deepEqual(onDisk.machines[0].fallbackUrls, []);
    });
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test("deleteMachine drops runtime endpoint state so a re-created machine with the same id starts fresh", async () => {
  const { dir, file } = tempMachinesFile();
  try {
    await withEnv({ OMP_WEB_MACHINES_FILE: file }, async () => {
      const subject = await loadSubject();
      const machine = subject.createMachine({
        ...
        authMode: "none",
        fallbackUrls: ["http://first-fallback.example"],
      });
      assert.equal(subject.deleteMachine(machine.id), true);

      const recreated = subject.createMachine({
        id: "reused-id",
        name: "Second",
        baseUrl: "http://second.example",
        authMode: "none",
      });
      const safe = subject.toSafeMachine(recreated);
      assert.equal(safe.activeUrl, "http://second.example");
      assert.deepEqual(safe.endpoints, [{ url: "http://second.example", healthy: null }]);
    });
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
