...
// The wrapper subscribes to the SDK event bus for async-subagent frames
// (same stub as rpc-manager-set-model.test.mjs).
const eventBus = { on: () => () => {} };
...
test("session shutdown notifies extensions before disposing the SDK session", async () => {
  ...
  const wrapper = new AgentSessionWrapper(inner, eventBus);
