...
test("resume-sourced session whose project differs from the stored project also reconciles (soft path)", async () => {
  const sessions = [
    session("a1", "/proj-a"),
    session("b1", "/proj-b"),
  ];
  const storage = createStorage({
    "omp-web:last-location": JSON.stringify({ v: 1, machine: "local", project: "/proj-a", session: "b1" }),
  });
  const { results, onChange } = collect();
  const deps = baseDeps({
    storage,
...
test("canonicalRewriteUrl: never rewrites a resume- or default-sourced settle, even when the URL clearly differs (no history pollution from opening '/')", () => {
  const settledFromResume = {
    phase: "settled",
    target: target("local", "/proj", "s1"),
    session: session("s1", "/proj"),
    error: null,
  ...
  assert.equal(canonicalRewriteUrl(settledFromResume, "/"), null);

  const settledFromDefault = { ...settledFromResume, source: "default" };
  assert.equal(canonicalRewriteUrl(settledFromDefault, "/"), null);
});
...
test("canonicalRewriteUrl: never rewrites a non-settled (mid-resolution) or error result", () => {
  const midResolution = { phase: "machines", target: target("local", null, null), session: null, error: null, source: "url" };
...
test("canonicalRewriteUrl end-to-end: a legacy-query-shaped run's settle rewrites the URL; a resume run's settle never does, even though its URL differs just as much", async () => {
  ...
  const { results: resumeResults, onChange: resumeOnChange } = collect();
  const storage = createStorage({
    "omp-web:last-location": JSON.stringify({ v: 1, machine: "local", project: "/proj", session: "s1" }),
  });
  createNavigationResolver(resumeOnChange).run(
    { kind: "resume" },
    baseDeps({ storage, listSessions: async () => sessions, getSession: async (_id, sid) => (sid === "s1" ? sessions[0] : null) }),
  );
  await flush();
  const resumeSettled = resumeResults[resumeResults.length - 1];
  assert.equal(resumeSettled.phase, "settled");
  assert.deepEqual(resumeSettled.target, urlSettled.target); // same destination...
  assert.equal(canonicalRewriteUrl(resumeSettled, "/"), null); // ...but resume never rewrites
});
...
test("default project pick falls back to no project when every candidate is removed (never an error — soft/default sourced)", async () => {
...
test("no removedProjectsSupplier -> nothing is excluded (matches every pre-existing test above)", async () => {
  ...
  const { results, onChange } = collect();
  const resolver = createNavigationResolver(onChange);
  resolver.run({ kind: "resume" }, baseDeps({ listSessions: async () => sessions }));
  await flush();
