...
import { parseLocation } from "./nav-url.ts";
...
test("resolveIntent: a url target defaults selection unless an explicit machine-shell intent is supplied", () => {
  const intent = resolveIntent({ kind: "target", target: target("m1", "/p", "s1") });
  assert.deepEqual(intent, { target: target("m1", "/p", "s1"), source: "url", home: false, selection: "defaults" });
});

test("resolveIntent: select-nothing remains explicit only for a blank machine target", () => {
  assert.deepEqual(
    resolveIntent({ kind: "target", target: target("remote-1", null, null) }, { selection: "none" }),
    { target: target("remote-1", null, null), source: "url", home: false, selection: "none" },
  );
  assert.deepEqual(
    resolveIntent({ kind: "target", target: target("remote-1", "/proj", null) }, { selection: "none" }),
    { target: target("remote-1", "/proj", null), source: "url", home: false, selection: "defaults" },
  );
});
...
test("machine-shell intent: a remote blank target commits the machine and settles with nothing selected", async () => {
  const sessions = [
    session("old", "/repo-a", { modified: "2026-01-01T00:00:00.000Z" }),
    session("new", "/repo-b", { modified: "2026-02-01T00:00:00.000Z" }),
  ];
  let sessionListCalls = 0;
  let lastOpenLookups = 0;
  const { results, onChange } = collect();
  const deps = baseDeps({
  ...
  const resolver = createNavigationResolver(onChange);
  resolver.run(
    { kind: "target", target: target("remote-1", null, null) },
    deps,
    { selection: "none" },
  );
  await flush();
  ...
  assert.deepEqual(settled.target, target("remote-1", null, null));
  assert.equal(settled.session, null);
  assert.equal(sessionListCalls, 1, "the remote session snapshot still loads before the shell mounts");
  assert.equal(lastOpenLookups, 0, "select-nothing must not consult a default session");
  assert.equal(canonicalRewriteUrl(settled, "/m/remote-1"), null, "the bare-machine URL remains stable");
});

test("legacy ?machine= deeplink retains default selection instead of becoming a machine shell", async () => {
  const sessions = [session("recent", "/legacy-project", { modified: "2026-02-01T00:00:00.000Z" })];
  const parsed = parseLocation("/", "?machine=remote-1");
  assert.deepEqual(
    resolveIntent(parsed),
    { target: target("remote-1", null, null), source: "url", home: false, selection: "defaults" },
  );

  const { results, onChange } = collect();
  createNavigationResolver(onChange).run(
    parsed,
    baseDeps({
      listMachines: async () => [{ id: "local" }, { id: "remote-1" }],
      listSessions: async () => sessions,
  ...
  assert.deepEqual(
    ...
    target("remote-1", "/legacy-project", "recent"),
    "legacy machine links still land on their remembered/default conversation",
  );
});
...
test("machine-shell intent preserves the not-found, no-permission, and offline machine error taxonomy", async () => {
  for (const variant of ["not-found", "no-permission", "offline"]) {
    const { results, onChange } = collect();
    createNavigationResolver(onChange).run(
      { kind: "target", target: target("unavailable", null, null) },
      baseDeps({ probeMachine: async () => variant }),
      { selection: "none" },
    );
    await flush();

    assert.deepEqual(results[results.length - 1].error, { stage: "machine", variant });
  }
});
...
test("select-nothing never suppresses defaultSession for a project deeplink without a session", async () => {
  const sessions = [
    session("older", "/proj", { modified: "2026-01-01T00:00:00.000Z" }),
    session("newer", "/proj", { modified: "2026-03-01T00:00:00.000Z" }),
  ];
  const { results, onChange } = collect();
  createNavigationResolver(onChange).run(
    { kind: "target", target: target("local", "/proj", null) },
    baseDeps({ listSessions: async () => sessions }),
    { selection: "none" },
  );
  await flush();

  assert.deepEqual(results[results.length - 1].target, target("local", "/proj", "newer"));
});
