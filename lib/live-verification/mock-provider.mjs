...
/** @typedef {"ok" | "unauthorized" | "forbidden"} ModelBehavior */

export function startMockProvider() {
  /** @type {Map<string, ModelBehavior>} */
  const behavior = new Map();
  /** @type {Array<{ model: string; mode: ModelBehavior; auth: string | null }>} */
  const requestLog = [];
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      requestLog.push({ model, mode, auth: req.headers.get("authorization") });
      ...
      if (mode === "forbidden") {
        return Response.json(
          {
            error: {
              message: `AccessDenied.Unpurchased: you have not purchased model "${model}".`,
              type: "AccessDenied",
