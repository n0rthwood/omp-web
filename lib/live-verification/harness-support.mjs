 ...
 * `extraFallbackChains` adds per-`provider/model` chain keys — the nested
 * chains whose reachability the #64 cascade harness verifies — and
 * `maxRetries` raises the per-turn hop budget when a scenario walks more
 * models than the default three.
 */
export function writeConfigYaml(
  agentDir,
  { defaultModel, fallbackChain, extraFallbackChains = {}, maxRetries = 3, keepRecentTokens = 20000 },
) {
  assertIsolatedAgentDir(agentDir);
  const chainLines = fallbackChain.map((m) => `      - ${m}`).join("\n");
  const extraChainLines = Object.entries(extraFallbackChains)
  ...
  writeFileSync(
    ...
    [
      ...
      "    default:",
      chainLines,
      ...(extraChainLines ? [extraChainLines] : []),
      "compaction:",
      "  enabled: true",
