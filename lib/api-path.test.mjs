...
test("appUrl emits the path form, keeping the machine, project, and session in sync", () => {
  setCurrentMachineId(LOCAL_MACHINE_ID);
  assert.equal(appUrl({}), "/m/local");
  assert.equal(appUrl({ project: "/home/alice" }), `/p/${encodeURIComponent("/home/alice")}`);
  assert.equal(appUrl({ project: "/home/alice", session: "s1" }), `/p/${encodeURIComponent("/home/alice")}/s/s1`);
  assert.equal(appUrl({ project: null, session: "s1" }), "/m/local"); // session dropped without a project

  setCurrentMachineId("boxtwo");
  assert.equal(appUrl({}), "/m/boxtwo");
  assert.equal(appUrl({ project: "/home/alice" }), `/m/boxtwo/p/${encodeURIComponent("/home/alice")}`);
  assert.equal(
    appUrl({ project: "/home/alice", session: "s1" }),
    `/m/boxtwo/p/${encodeURIComponent("/home/alice")}/s/s1`,
  );
  assert.equal(appUrl({ project: "/home/alice", session: null }), `/m/boxtwo/p/${encodeURIComponent("/home/alice")}`);
  // An explicit id overrides the current machine, for the switch itself.
  assert.equal(appUrl({}, LOCAL_MACHINE_ID), "/m/local");
});

test("appUrl encodes a machine id that needs it", () => {
  setCurrentMachineId(LOCAL_MACHINE_ID);
  assert.equal(appUrl({}, "we ird/id"), `/m/${encodeURIComponent("we ird/id")}`);
});
