...
test("round-trip: /m/local", () => {
  const target = { machineId: "local", project: null, session: null };
  const url = buildUrl(target);
  assert.equal(url, "/m/local");
  assert.deepEqual(parseLocation(url, ""), { kind: "target", target });
});

test("root remains the Home entry", () => {
  assert.deepEqual(parseLocation("/", ""), { kind: "home" });
});
...
test("malformed: /m/x/s/y (session without /p/) -> home", () => {
  assert.deepEqual(parseLocation("/m/x/s/y", ""), { kind: "home" });
});

test("malformed: /p/x/z/y (extra unexpected segment) -> home", () => {
  assert.deepEqual(parseLocation("/p/x/z/y", ""), { kind: "home" });
});

test("malformed: trailing junk after a full /m/.../p/.../s/... shape -> home", () => {
  assert.deepEqual(parseLocation("/m/x/p/y/s/z/extra", ""), { kind: "home" });
});

test("malformed: /m/x/p (empty project segment, nothing after p) -> home", () => {
  assert.deepEqual(parseLocation("/m/x/p", ""), { kind: "home" });
});

test("malformed: /p//s/y (empty project segment via double slash) -> home", () => {
  assert.deepEqual(parseLocation("/p//s/y", ""), { kind: "home" });
});

test("malformed: /x (unrecognized top segment) -> home", () => {
  assert.deepEqual(parseLocation("/x", ""), { kind: "home" });
});

test("malformed: undecodable percent-sequence -> home, never throws", () => {
  assert.deepEqual(parseLocation("/p/%E0%A4%A", ""), { kind: "home" });
});

test("malformed: leading double slash -> home, never throws", () => {
  assert.deepEqual(parseLocation("//m/x", ""), { kind: "home" });
});

test("session-without-project via buildUrl is dropped defensively", () => {
  const target = { machineId: "local", project: null, session: null };
  const url = buildUrl({ ...target, session: "orphan" });
  assert.equal(url, "/m/local");
  assert.deepEqual(parseLocation(url, ""), { kind: "target", target });
});
...
// --- bare /m and /p and / -> home (issue #15: Always Home) ---------------------

test("bare /m -> home", () => {
  assert.deepEqual(parseLocation("/m", ""), { kind: "home" });
});

test("bare /p -> home", () => {
  assert.deepEqual(parseLocation("/p", ""), { kind: "home" });
});

test("bare /m/ and /p/ (trailing slash) -> home", () => {
  assert.deepEqual(parseLocation("/m/", ""), { kind: "home" });
  assert.deepEqual(parseLocation("/p/", ""), { kind: "home" });
});
...
test("buildUrl omits the local machine segment only when a project is present", () => {
  // Project-less local stays explicit, so it never collides with Home ("/").
  assert.equal(buildUrl({ machineId: "local", project: null, session: null }), "/m/local");
  assert.equal(buildUrl({ machineId: "local", project: "proj", session: null }), "/p/proj");
  assert.equal(buildUrl({ machineId: "local", project: "proj", session: "s1" }), "/p/proj/s/s1");
});
...
test("legacy: empty-string values are treated as absent", () => {
  ...
  assert.deepEqual(parseLocation("/", search), { kind: "home" });
});
