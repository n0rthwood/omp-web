...
test("sanitizeNextPath accepts /m/... and /p/... deeplinks unchanged, still rejects protocol-relative junk", async () => {
  const { sanitizeNextPath } = await import("./login-next-path.ts");
  assert.equal(sanitizeNextPath("/m/box2/p/%2Fhome%2Falice/s/sess-1"), "/m/box2/p/%2Fhome%2Falice/s/sess-1");
  assert.equal(sanitizeNextPath("/p/%2Fhome%2Falice"), "/p/%2Fhome%2Falice");
  assert.equal(sanitizeNextPath("//m/x"), "/");
  assert.equal(sanitizeNextPath("//p/x"), "/");
});
...
test("web-me: null user when unauthenticated, real user with a session cookie, flag when disabled", async () => {
  ...
  await withStore({ OMP_WEB_PASSWORD: undefined }, async () => {
    ...
    assert.deepEqual(await res.json(), {
      user: { username: "__anonymous", role: "admin", visibleProjects: "*", machines: "*" },
      authRequired: false,
    });
  });
});
...
test("env-backed migration admin logs in with OMP_WEB_PASSWORD", async () => {
  await withStore({ OMP_WEB_PASSWORD: "env-secret-pw" }, async () => {
    ...
    assert.equal(res.status, 200);
    assert.deepEqual(await res.json(), {
      user: { username: "omp", role: "admin", visibleProjects: "*", machines: "*" },
    });
