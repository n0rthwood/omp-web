...
const ALICE = { username: "alice", role: "user", projects: ["/home/alice"], machines: [] };
...
test("resolves the env-backed admin via legacy Basic while the env password is set", async (t) => {
  ...
  await withEnv(
    ...
    async () => {
      ...
      assert.deepEqual(user, { username: "omp", role: "admin", visibleProjects: "*", machines: "*" });
    },
  );
});

test("legacy Basic stops granting admin once a file user claims 'omp'", async (t) => {
  const store = tempStore();
  t.after(() => rmSync(store.dir, { recursive: true, force: true }));
  await withEnv(
    {
    ...
    async () => {
      ...
      const { getWebUserFromRequest } = await loadAuthContext();
      // The env password no longer yields admin: the file user owns the name.
      assert.equal(
...
test("filterVisibleSessions keeps sessions whose cwd or projectRoot is visible", async () => {
  ...
  const sessions = [
    ...
    // Worktree sessions carry the main repo's projectRoot while the cwd is
    // the worktree: either anchor being visible is enough (issue #7).
    { id: "5", cwd: "/home/alice", projectRoot: "/home/bob" },
  ];
  assert.deepEqual(
