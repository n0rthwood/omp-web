...
const { findFirstUserMessage, findFirstTitleSource, truncateTitle, isDeclinedTitle, normalizeWebGeneratedTitle, appendIssueAnnotationSuffix, shouldAutoGenerateTitle } = await jiti.import("./session-title.ts");
...
test("collapses whitespace and caps the title length", () => {
  ...
  assert.equal(Array.from(truncateTitle("x".repeat(200))).length, 120);
});

test("normalizeWebGeneratedTitle accepts SDK-rejected 81-120 character web titles", () => {
  const title = "x".repeat(100);
  assert.equal(normalizeWebGeneratedTitle(`<title>${title}</title>`), title);
});

test("normalizeWebGeneratedTitle caps web titles at 120 characters", () => {
  const normalized = normalizeWebGeneratedTitle(`<title>${"x".repeat(140)}</title>`);
  assert.equal(Array.from(normalized).length, 120);
});

test("appendIssueAnnotationSuffix adds literal issue refs after local human-only titles", () => {
  assert.equal(
    appendIssueAnnotationSuffix("Fix login redirect", "Please fix #12; related context rel #10 and rel #7."),
    "Fix login redirect (#12 · rel #10, #7)",
  );
});

test("appendIssueAnnotationSuffix keeps local fallback titles within the 120 character web cap", () => {
  const result = appendIssueAnnotationSuffix("x".repeat(115), "Fix #14.");
  assert.equal(Array.from(result).length, 120);
  assert.match(result, /\(#14/);
});

test("appendIssueAnnotationSuffix does not title a session from issue refs alone", () => {
  assert.equal(appendIssueAnnotationSuffix("", "Fix #14."), "");
});
