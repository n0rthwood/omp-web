...
test("uses a service worker notification when a registration is available", async () => {
  ...
  const delivery = await showBrowserNotification({
    ...
    body: "Task finished.",
    // A realistic AppShell-built URL (buildUrl, issue #10 stage-3 review
    // blocker #3) — this module only ever passes it through opaquely, but
    // the fixture should look like what a caller actually sends: machine-
    // qualified, not the old machine-less `?session=` legacy shape.
    sessionUrl: "/m/box2/p/%2Fhome%2Falice%2Fproject/s/session-1",
  ...
  assert.deepEqual(shown, [{
    ...
    options: {
      body: "Task finished.",
      tag: "session-complete:session-1",
      data: { url: "/m/box2/p/%2Fhome%2Falice%2Fproject/s/session-1" },
    },
  }]);
});
...
test("falls back to a page notification and wires its click handler", async () => {
  ...
  const delivery = await showBrowserNotification({
    title: "Session complete",
    body: "Task finished.",
    sessionUrl: "/p/%2Fhome%2Falice%2Fproject/s/session-1",
