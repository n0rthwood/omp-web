...
test("#64: a failing fallback model cascades into its OWN chain's next model (nested chains reachable)", async () => {
  const before = snapshotRealAgentState();
  const { home, cwd } = buildIsolatedHome("nested-chain-cascade");
  try {
    const { exitCode, result, stderr } = await runHarness(harnessScript("nested-chain-cascade-harness.mjs"), {
      home,
      cwd,
      resultFile: join(home, "result.json"),
      timeoutMs: 45_000,
    ...
    // Turn 1 resolves to the configured primary, no fallback.
    assert.equal(result.beforeKillModelChangeCount, 1);
    assert.deepEqual(result.allModelChanges[0], { model: "mock/model-primary", resolvedModelIsFallback: false });

    // The cascade order, hop by hop, from the first failed primary request:
    // primary 401 → chain entry x 403 → chain entry a (LAST of default)
    // 403 → a's OWN chain key's entry c, which answers. On the sticky-key
    // bug (SDK <= 17.3.0, joysort109 incident) the walk stops at a — c is
    // never requested.
    assert.deepEqual(result.cascade, [
      { model: "model-primary", mode: "unauthorized" },
      { model: "model-x", mode: "forbidden" },
      { model: "model-a", mode: "forbidden" },
      { model: "model-c", mode: "ok" },
    ]);

    // The turn actually COMPLETED on the nested chain's model — not just
    // switched and then errored.
    assert.ok(result.fallbackModelChanges.includes("mock/model-c"), `fallback changes: ${result.fallbackModelChanges}`);
    assert.match(result.lastAssistantText, /model-c/);
    assert.equal(result.lastAssistantStopReason, "stop");
  } finally {
    cleanup(home, cwd);
  }
